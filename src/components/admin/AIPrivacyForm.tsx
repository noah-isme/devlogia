  ...
  return (
    <section className="space-y-6 rounded-xl border border-border/60 bg-muted/30 p-4">
      ...
      <div className="flex flex-wrap gap-3 pt-2">
        ...
        <button
          type="button"
          className="rounded-md border border-primary bg-primary px-3 py-2 text-sm font-medium text-primary-foreground transition hover:opacity-90"
          onClick={async () => {
            setLoading(true);
            setMessage(null);
            try {
              window.location.href = "/api/admin/export";
              setMessage("Backup export initialized!");
            } catch (error) {
              setMessage(error instanceof Error ? error.message : "Full backup failed");
            } finally {
              setLoading(false);
            }
          }}
