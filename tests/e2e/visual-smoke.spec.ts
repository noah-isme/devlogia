...
async function createPageViaApi(page: Page, payload: PagePayload) {
  ...
  if (!createdPage?.id || !createdPage.slug || !createdPage.title) {
    throw new Error(
      "Page creation response did not include id, slug, and title.",
    );
  }
...
test("visual smoke screenshots", async ({ page }, testInfo) => {
  ...
  await attachScreenshot(page, testInfo, "admin-login-mobile");
  ...
  await expect(
    page.getByRole("heading", { name: /content health/i }),
  ).toBeVisible();
  ...
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: /publish smarter/i }),
  ).toBeVisible();
  await attachScreenshot(page, testInfo, "landing");

  await page.goto("/blog");
  await expect(
    page.getByRole("heading", { name: /ideas for people building/i }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: postTitle }).first(),
  ).toBeVisible();
  await attachScreenshot(page, testInfo, "blog-detail");

  await page.goto("/subscribe");
  await expect(
    page.getByRole("heading", { name: /one thoughtful email/i }),
  ).toBeVisible();
  await attachScreenshot(page, testInfo, "subscribe");

  await page.goto(`/${createdPage.slug}`);
  await expect(
    page.getByRole("heading", { level: 1, name: pageTitle }).first(),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", { name: /team roles/i }),
  ).toBeVisible();
  ...
  await expect(
    page
      ...
      .or(page.getByTestId("analytics-error")),
  ).toBeVisible();
  ...
  await expectResponsivePage(page);
  await attachScreenshot(page, testInfo, "blog-index-mobile");
  ...
  await expectResponsivePage(page);
  await attachScreenshot(page, testInfo, "blog-detail-mobile");
